...
%     points_path_temp = path_refine_samedirection(path);
%     points_path_temp = path_refine_closepts(path, 10*1.4*resxy_persistent, 10*1.42*resxy_persistent);
%     path = path_refine_samedirection(path);
...
%     path_n = points_path;
   ...
   % **************** Generate interpolation matrixs* *********************
     snap_coeffs = Min_SnapSpline(time_matrix, path_processed, zeros(6,3));
...
% ******************************* Initial *********************************
size_pt = size(path, 1);
snap_matrix = zeros(8 * (size_pt - 1),8 * (size_pt - 1)); 
% for j = 1:3
    snap_matrix(:,:) = eye(8 * (size_pt - 1))*eps;
% end

state_matrix = zeros(8 * (size_pt - 1), 3);
...
%     xd4 = [0, 0, 0, 0, 24, 80*t];
    ...
    if i == 1
        ...
        state_matrix(1:4,:) = [path(i,:); state_SF(1:3, :)];
    elseif i == size_pt
        snap_matrix(end-3:end, end-7:end) = [xt; xd; xdd; xddd];
        state_matrix(end-3:end, :) = [path(i,:); state_SF(4:6, :)];
%     if i == 1
%         snap_matrix(1:3, 1:6) = [xt; xd; xdd];
%         state_matrix(1:3,:) = [path(i,:); zeros(2,3)];
%     elseif i == size_pt
%         snap_matrix(end-2:end, end-5:end) = [xt; xd; xdd];
...
% ********************* determine the segment *****************************
    ...
    coeff = coeff_matrix(8*(row_need - 1)-7:8*(row_need - 1), :);
