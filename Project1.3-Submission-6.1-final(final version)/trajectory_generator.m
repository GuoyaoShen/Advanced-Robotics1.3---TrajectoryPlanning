...
function collision = check_collision(start, goal, map)
    ...
    newblocks(:, 1:3) = obstacle(:, 1:3) - margin_safe;
    newblocks(:, 4:6) = obstacle(:, 4:6) + margin_safe;
...
function path_new = path_refine_deletepts(path, map)
    dist = sqrt(sum(diff(path).^2, 2));  %(n-1)x1 matrix
    dist_sum = sum(dist);  %sum distance
    if dist_sum > 60
        range = 7;
    else
        range = 5;
    ...
    while ( ~isequal(path_new(end, :), path(end, :)))      
        ...
        for j = 1:size_path
            ...
            if check_collision(path_new(end,:), path(size_path-j+1, :),...
                map) == 0 && sqrt(sum((path_new(end,:)- path(size_path-j+1, :)).^2,2))<range
